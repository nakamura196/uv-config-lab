...
**点検はコレクション単位で行ってください。** 同じサイト内でも、別のコレクションでは
`service @id` が最初から正規形のことがあります（実測で確認）。1 資料だけ見て
「問題なし」と結論すると取りこぼします。
...
1. **サーバ側** — 識別子を書き換えている逆プロキシで **`X-Forwarded-ID`** を立て、`info.json` の
   `@id` をマニフェストに合わせる。外から付けても効きません（効いてはいけません）ので、
   確認はサーバ側でしかできません。
   **`X-IIIF-ID` は Cantaloupe 5.0 で削除済みです**（3.2 で追加 → 4.0 で `X-Forwarded-ID` に
   置き換えて非推奨 → 5.0 でサポート終了）。対象が 5.x なら `X-IIIF-ID` を立てても効きません
